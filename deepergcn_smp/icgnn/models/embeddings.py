...
def ppr(adj, alpha=0.15, normalization='symmetric') -> np.ndarray:
    ...
    if sp.issparse(adj):
        ...
        raise ValueError(f"adj must be a sparse matrix or numpy array (got {type(adj)} instead)")
    eps = 1e-6
    deg = adj.sum(1) + eps
...
def full_verse(adj, emb_dim=64, similarity='ppr', ppr_alpha=0.85,
               learning_rate=0.005, max_epochs=2000, verbose=False, l2=False):
