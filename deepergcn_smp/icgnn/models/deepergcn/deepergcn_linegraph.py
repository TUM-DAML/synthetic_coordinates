...
class DeeperGCN_LineGraph(torch.nn.Module):
    ...
    def __init__(self, num_tasks=None, 
                num_layers=7, dropout=0.2, block='res+',
                hidden_channels=256, conv='gen', 
                gcn_aggr='softmax', 
                learn_t=True, t=1.0, learn_p=False, p=1,
                msg_norm=False, learn_msg_scale=False, norm='batch', mlp_layers=1, 
                graph_pooling='mean',
        ...
        self.msg_emb_layer = nn.Linear(2 * hidden_channels + edge_attr_dim \
                                      + lg_node_basis, 
                                        hidden_channels)
        ...
        if emb_basis_global:
            if emb_bottleneck:
                if emb_basis_local:
                    ...
                    self.global_emb_node_basis = nn.Linear(lg_node_basis, emb_bottleneck)
                    self.global_emb_edge_basis = nn.Linear(lg_edge_basis, emb_bottleneck)
                else:
        ...
        for _ in range(num_layers):
            if conv == 'gen':
                gcn = GENConv_Linegraph(hidden_channels, hidden_channels,
                              aggr=gcn_aggr,
                              t=t, learn_t=learn_t,
                              p=p, learn_p=learn_p,
                              msg_norm=msg_norm, 
    ...
    def forward(self, batch):
        ...
        final_node_emb = torch_scatter.scatter_add(h, batch.edge_index_g[1],
                        dim=0, dim_size=batch.x_g.shape[0]) # this is graph.num_nodes
