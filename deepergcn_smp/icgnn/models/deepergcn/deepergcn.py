...
class DeeperGCN(torch.nn.Module):
    def __init__(self, num_tasks, num_layers=7, dropout=0.2, block='res+', conv_encode_edge=False, 
                add_virtual_node=False, hidden_channels=256, conv='gen', 
                gcn_aggr='softmax', learn_t=True, t=1.0, learn_p=False, p=1,
                msg_norm=False, learn_msg_scale=False, norm='batch', mlp_layers=1, graph_pooling='mean',
                edge_feat_dim=None, mol_data=True, code_data=False, node_encoder=None,
                node_feat_dim=7, emb_product=True, mlp_act='relu', emb_use_both=False,
                deg_hist=None, emb_bottleneck=False, emb_use_global=True):
        ...
        for layer in range(self.num_layers):
            if conv == 'pna':
                gcn = PNAConv(hidden_channels, hidden_channels,
                ...
                gcn = GENConv(hidden_channels, hidden_channels,
                              ...
                              norm=norm, mlp_layers=mlp_layers, emb_product=emb_product,
                              mlp_act=mlp_act, 
        ...
        if self.code_data:
            ...
            for i in range(self.num_tasks):
                 self.graph_pred_linear_list.append(torch.nn.Linear(hidden_channels, 5000 + 2))
        else:
