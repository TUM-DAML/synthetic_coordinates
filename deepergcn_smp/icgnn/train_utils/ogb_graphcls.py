...
def train_eval_model(model, loaders, optim, evaluator, max_epochs, device, task_type,
                    eval_metric, criterion, multi_class,
                    warmup=None, scheduler=None, min_lr=None,
                    max_hours=None, logdir='logs/output'):
    ...
    for epoch in range(max_epochs):
        ...
        train_loss, train_result = step(True, model, device, loaders['train'], 
                                        multi_class, criterion, evaluator, optim)
        val_loss, val_result = step(False, model, device, loaders['val'], 
                                        multi_class, criterion, evaluator)
        ...
        print(f'Epoch:{epoch} tloss: {train_loss:.4f} vloss: {val_loss:.4f} tmetric: {train_metric:.4f} vmetric: {val_metric:.4f}')
        ...
        if (task_type != 'regression' and val_metric > best_val) or (val_metric < best_val):
            best_val = val_metric
            final_test = evaluate(model, device, loaders['test'], evaluator, multi_class)[eval_metric]
...
def step(is_train, model, device, loader, multi_class, criterion, evaluator, optimizer=None,
        ):
    loss_list = []
    y_true = []
    y_pred = []
