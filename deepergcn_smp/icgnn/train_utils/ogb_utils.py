...
def ogbgcode_arr_to_seq(arr):
    pass


def get_ogbgcode_encoder(dataset_root, emb_dim):
    nodetypes_mapping = pd.read_csv(
        os.path.join(dataset_root, "mapping", "typeidx2type.csv.gz")
    )
    nodeattributes_mapping = pd.read_csv(
        os.path.join(dataset_root, "mapping", "attridx2attr.csv.gz")
    )
