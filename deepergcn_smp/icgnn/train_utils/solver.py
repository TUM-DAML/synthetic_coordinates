...
def solve(
    ...
    for seed_ndx, seed in enumerate(seeds):
        ...
        graph = set_train_val_test_split(seed, graph, num_development=1500).to(device)
        ...
        for epoch in range(max_epochs):
            ...
            if epoch % display_step == 0:
                print(
                    f"Epoch: {epoch:03d}, \t T.acc: {train_acc:.4f}, V.acc: {val_acc:.4f}               T.loss: {train_loss:.4f}, V.loss: {val_loss:.4f}"
                )
    ...
    boots_series = sns.algorithms.bootstrap(
        best_dict["val_acc"], func=np.mean, n_boot=1000
    )
    best_dict["val_acc_ci"] = np.max(
        np.abs(sns.utils.ci(boots_series, 95) - np.mean(best_dict["val_acc"]))
    )
...
def solve_graphcls(
    ...
    model.to(device).reset_parameters()
    optimizer = torch.optim.Adam(
        model.parameters(), lr=learning_rate, weight_decay=l2_reg, amsgrad=use_amsgrad
    )
    ...
    for epoch in range(max_epochs):
        ...
        train_loss, train_acc = step_graphcls(
            model,
            train_loader,
            optimizer,
            is_train=True,
        ...
        with torch.no_grad():
            val_loss, val_acc = step_graphcls(
                model,
                val_loader,
                optimizer=None,
                is_train=False,
...
def step_graphcls(
    model, loader, optimizer=None, is_train=False, binary_cls=False, ogb_evaluator=None
):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
