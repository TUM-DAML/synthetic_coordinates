...
class Add_Linegraph(object):
    ...
    def __call__(self, data):
        ...
        '''
        graph_copy = Data(edge_index=data.edge_index.clone(),
        ...
        # remove self loops
        noloop_mask = ~(data.edge_index[0, linegraph.edge_index[0]] == data.edge_index[1, linegraph.edge_index[1]])
        ...
        new_data = ICGNN_Data(x_g=data.x, edge_index_g=data.edge_index,
        ...
        all_attrs = 'x', 'edge_index', 'edge_attr', 'y', 'distances', \
            'edge_dist', 'max_edge_dist', 'min_edge_dist', 'max_dist', \
            'min_dist', 'edge_dist_basis'
...
class Set_PPR_Distance(object):
    ...
    def __call__(self, data):
        ...
        data.edge_dist_basis = get_dist_basis(edge_lengths, type=self.dist_basis_type,
...
def get_min_basis(data, n_basis):
    ...
    Get min angle basis
    '''
    angle = angle_from_dists(data.edge_index_lg.T, data.edge_index_g.T, 
                    data.max_edge_dist, data.max_edge_dist, data.min_dist)
...
def get_max_basis(data, n_basis):
    ...
    '''
    angle = angle_from_dists(data.edge_index_lg.T, data.edge_index_g.T,
                     data.min_edge_dist, data.min_edge_dist, data.max_dist)
    basis = get_cos_basis(angle, n_basis)
...
def get_center_basis(data, basis):
    ...
    angle = angle_from_dists(data.edge_index_lg.T, data.edge_index_g.T,
                                 mean_edge_dist, mean_edge_dist, mean_dist)
...
def get_single_basis(data, n_basis):
    angle = angle_from_dists(data.edge_index_lg.T, data.edge_index_g.T, 
                                data.edge_dist, data.edge_dist, data.distances)
...
class Set_Linegraph_EdgeAttr_Angle(object):
    ...
    def get_ppr_data(self, data):
        ...
        '''
        return Data(edge_index_g=data.edge_index_g,
