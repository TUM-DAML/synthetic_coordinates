...
class Set_Edge_Dist:
    ...
    def __call__(self, data):
        ...
        data.edge_dist_basis = get_dist_basis(edge_lengths, self.dist_basis_type, self.num_dist_basis)
...
class Set_BoundsMatBoth_Distance:
    ...
    '''
    def __init__(self, num_dist_basis=4, dist_basis_type='gaussian',
                metric_graph_cutoff=None, metric_graph_edgeattr=None):
        '''
        ...
        if self.metric_graph_cutoff:
            print(f'(transform) BM-both: Using metric graph. Edge attr method: {self.metric_graph_edgeattr}')
    ...
    def __call__(self, sample):
        ...
        if self.metric_graph_cutoff:
            ...
            graph.edge_index = torch.LongTensor(get_radius_graph_from_distmat(min_dist,
                                                        self.metric_graph_cutoff))
            ...
            # new edges: 0 0 0.. + 1
                ...
                # matrix of 0s
                new_edge_attr = torch.zeros((graph.num_nodes, graph.num_nodes, 
                                            edge_attr_dim + 1))
        ...
        if self.num_dist_basis > 2:
            max_dist_basis = get_dist_basis(max_edge_lengths, self.dist_basis_type, self.num_dist_basis // 2, max_dist=4)
            min_dist_basis = get_dist_basis(min_edge_lengths, self.dist_basis_type, self.num_dist_basis // 2, max_dist=4)
