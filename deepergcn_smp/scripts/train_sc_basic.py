...
result = run(dataset_name='ZINC', 
        model='smp',
        # optim params
        lr_schedule=True, min_lr=1e-5, patience=10, 
        max_epochs=3000, learning_rate=1e-4, l2_reg=0, batch_size=128,
        lr_warmup=False,
        # random seed
        seed=0,
        # our new features
        # set to True to use PPR
        add_ppr_dist=False,
        ...
        # model params
        mlp_act='relu',
        num_layers=12, dropout=0.2, block='res+', conv_encode_edge=True, 
        add_virtual_node=False, hidden_channels=128, conv='gen', 
        gcn_aggr='softmax', learn_t=True, t=0.1, learn_p=False, p=1,
