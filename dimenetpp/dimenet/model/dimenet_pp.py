...
class DimeNetPP(tf.keras.Model):
    ...
    def __init__(
            ...
            cutoff=5.0, num_before_skip=1,
            num_after_skip=2, num_dense_output=3, num_targets=12,
            activation=swish, output_init='zeros', name='dimenet', 
            dist={'type': 'none'}, **kwargs):
        ...
        self.sbf_layer = SphericalBasisLayer(num_spherical, num_radial, cutoff=cutoff, 
                                            split=sph_split)
        ...
        self.edge_embeddings = tf.keras.layers.Embedding(
                4, emb_size, embeddings_initializer=emb_init,
            )                                               
