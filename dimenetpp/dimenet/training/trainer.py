...
class Trainer:
    def __init__(
        ...
        self.opt = tf.optimizers.Adam(learning_rate=self.learning_rate, amsgrad=True)
        self.optimizer = tfa.optimizers.MovingAverage(
            self.opt, average_decay=self.ema_decay
        )
    ...
    def update_weights(self, loss, gradient_tape):
        ...
        if self.max_grad_norm is not None:
            grads, _ = tf.clip_by_global_norm(
                grads, self.max_grad_norm, use_norm=global_norm
            )
