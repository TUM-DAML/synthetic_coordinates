...
index_keys = ["batch_seg", "idnb_i", "idnb_j",
              "id_expand_kj", "id_reduce_ji"]


class DataContainer:
    def __init__(self, filename, target_keys, dist={'type': 'none'}, subset=False,
                ablation=None):
        ...
        pyg_qm9_mean = np.array([ 2.6821668e+00,  7.5270493e+01, -6.5350609e+00,  3.2208392e-01,
                 6.8571439e+00,  1.1894536e+03,  4.0546360e+00, -1.1182017e+04,
                -1.1181785e+04, -1.1181761e+04, -1.1182925e+04,  3.1616858e+01,
                -7.6077148e+01, -7.6541321e+01, -7.6978897e+01, -7.0799980e+01,
                 9.9633703e+00,  1.4066898e+00,  1.1270893e+00])
        ...
        conversion_all = np.array([  1.5034491 ,   8.17294675,   0.59772825,   1.27480012,
        ...
        for _, mol in enumerate(tqdm(self.data)):
            ...
            converted_target = np.array(mol['targets'])[target_idx] * conversion + targets_mean
    ...
    def _calculate_neighbor_angles_diff(self, dij, djk, dik, id3_i, id3_j, id3_k, eps=1e-6):
        ...
        angle = np.arccos(np.clip((D2ij + D2jk - D2ik) / (2 * Dij * Djk + eps), a_min=-1, a_max=1))
    ...
    def _calculate_neighbor_angles(self, dists, id3_i, id3_j, id3_k):
        ...
        angle = np.arccos(np.clip((D2ij + D2jk - D2ik) / (2 * Dij * Djk), a_min=-1, a_max=1))
    ...
    def __getitem__(self, idx):
        ...
        if self.dist['type'] in ('ppr', 'ppr_rdkit_bounds') :
            ...
            Anglesijk.extend([self._calculate_neighbor_angles(dists, id3dnb_i, id3dnb_j, id3dnb_k)])
            
        if self.dist['type'] in ('rdkit_bounds', 'ppr_rdkit_bounds') :
            min_dists = self._bmat_fast([mol['min_dist'] for mol in mols])
            max_dists = self._bmat_fast([mol['max_dist'] for mol in mols])
            mean_dists = self._bmat_fast([(mol['min_dist']+mol['max_dist'])/2 for mol in mols])
        ...
        if self.ablation in ('const_angle', 'const_both',):
