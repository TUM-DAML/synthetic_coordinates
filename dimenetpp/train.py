...
def run(model_name, emb_size, out_emb_size, int_emb_size, basis_emb_size,
        num_blocks, num_spherical, num_radial, output_init,
        num_before_skip, num_after_skip, num_dense_output,
        cutoff, dist, dataset, num_train, num_valid,
        ...
        comment, logdir, quick_run=False, ablation=None):
    if not quick_run:
    ...
    with summary_writer.as_default():
        ...
        data_containers = {
            'train': DataContainer(dataset + "/train.jsonl.gz", target_keys=targets, dist=dist,
                                    subset=quick_run, ablation=ablation),
            'val': DataContainer(dataset + "/valid.jsonl.gz", target_keys=targets, dist=dist,
                                    subset=quick_run, ablation=ablation),
            'test': DataContainer(dataset + "/test.jsonl.gz", target_keys=targets, dist=dist,
                                    subset=quick_run, ablation=ablation),
        }

        # Initialize DataProvider (splits dataset into 3 sets based on data_seed and provides tf.datasets)
        data_provider = DataProvider(data_containers, ntrain=num_train, nvalid=num_valid,
                                     batch_size=batch_size, seed=data_seed, dist=dist)
